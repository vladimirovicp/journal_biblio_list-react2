...
import { buildElibraryXml } from '@/utils/xmlBuilder'
import { downloadXml } from '@/utils/fileHelper'

interface ElibraryXmlSectionProps {
  siteUrl: string | null
...
export default function ElibraryXmlSection({
  siteUrl,
  journalNumberData,
  articleIds,
}: ElibraryXmlSectionProps) {
  const [xml, setXml] = useState<string | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [generatedKey, setGeneratedKey] = useState<string | null>(null)

  const key = siteUrl && journalNumberData && articleIds.length > 0
    ? `${siteUrl}-${journalNumberData.id}-${articleIds.join(',')}`
    : null
  ...
  useEffect(() => {
    ...
    fetchJournalMetadata(siteUrl)
        ...
        return articleIds.reduce<Promise<JournalArticleDetail[]>>(
          (chain, id, index) =>
            chain.then((results) => {
              ...
              console.log(`Загрузка статьи ${index + 1} из ${articleIds.length}...`)
      ...
      .then((articles) => {
        ...
        const result = buildElibraryXml(metadata, journalNumberData, articles)
        setXml(result)
        setGeneratedKey(
          `${siteUrl}-${journalNumberData.id}-${articleIds.join(',')}`,
        )
  ...
  const handleDownload = () => {
    ...
    const filename = `${journalNumberData.title.en}-elibrary.xml`
